...
// ... Dot component ...
const Dot = React.memo(({ index, sizeNum, color }: { index: number; sizeNum: number; color: string }) => {
    ...
    useEffect(() => {
        const moveIn = withTiming(1, { duration: STEP_DURATION, easing: Easing.inOut(Easing.ease) });
        const moveOut = withTiming(0, { duration: STEP_DURATION, easing: Easing.inOut(Easing.ease) });
...
export const Loading: React.FC<LoadingProps> = ({ 
    ...
    const Spinner = React.useMemo(() => (
        <Animated.View style={[{ width: sizeNum, height: sizeNum, justifyContent: 'center', alignItems: 'center' }, containerStyle]}>
    ...
    return (
        <Animated.View style={styles.container} exiting={FadeOut.duration(800)}>
            {Spinner}
        </Animated.View>
    );
};
