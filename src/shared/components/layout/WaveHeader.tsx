...
export const WaveHeader: React.FC<WaveHeaderProps> = ({
  ...
  const wave1AnimatedProps = useAnimatedProps(() => {
    'worklet';
    const path = createWavePath(width, svgHeight, waveHeight, amplitude, numPoints, phase1.value);
    return { d: path };
  });
  ...
  return (
    ...
    >
      ...
      <AnimatedSvg width={svgWidth} height={svgHeight} style={[wave2Style, styles.waveAbsolute]}>
        <AnimatedPath animatedProps={wave2AnimatedProps} fill={fill2} opacity={0.5} />
      </AnimatedSvg>

      {/* Sóng chính - rõ nét, màu fill1 */}
      <AnimatedSvg width={svgWidth} height={svgHeight} style={[wave1Style, styles.waveAbsolute]}>
