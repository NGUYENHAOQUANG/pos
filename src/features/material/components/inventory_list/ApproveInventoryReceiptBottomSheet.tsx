...
export interface ApproveInventoryReceiptBottomSheetProps {
    visible: boolean;
    onClose: () => void;
    id: string;
}
...
export const ApproveInventoryReceiptBottomSheet: React.FC<
    ApproveInventoryReceiptBottomSheetProps
> = ({ visible, onClose, id }) => {
    const theme = useAppTheme();
    const insets = useSafeAreaInsets();
    const styles = getStyles(theme, insets.bottom);

    const { data: item, isLoading, isError } = useInventoryDetail(visible ? id : '');
    const { data: inventoryItems, isLoading: isFetchingItems } = useInventoryItems(
    ...
    const materials = React.useMemo(() => inventoryItems || [], [inventoryItems]);
        ...
        (
            ...
            <View style={[styles.columnView, flexGrow ? { flexGrow } : undefined]}>
                ...
                {data.map((rowItem, idx) => (
                    <View
                        key={rowItem.id || rowItem.inventoryCheckItemId || idx}
                        style={[
            ...
            <View style={styles.container}>
                <View style={[styles.header, styles.contentPadding]}>
                    <View style={styles.headerLeft}>
                        <Text style={styles.title}>{item.checkCode || '---'}</Text>
                ...
                >
                    <View style={styles.infoSection}>
                        <DetailRow label="Loại phiếu:" value="Phiếu kiểm kê" />
                        <DetailRow label="Người tạo:" value={item.creator?.fullname || '---'} />
                        <DetailRow label="Kho kiểm kê:" value={item.warehouseName || '---'} />
                        <DetailRow
                            label="Ngày tạo:"
                            value={item.createdAt ? formatMaterialDateTime(item.createdAt) : '---'}
                        />
                        <DetailRow label="Ghi chú:" value={item.note || '---'} />
                    </View>
