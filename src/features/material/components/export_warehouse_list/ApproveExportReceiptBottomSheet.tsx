...
export interface ApproveExportReceiptBottomSheetProps {
    visible: boolean;
    onClose: () => void;
    id: string;
}

export const ApproveExportReceiptBottomSheet: React.FC<ApproveExportReceiptBottomSheetProps> = ({
    visible,
    onClose,
    id,
}) => {
    ...
    const { data: item, isLoading, isError } = useExportReceipt(visible ? id : '');
    const { data: exportItems, isLoading: isFetchingItems } = useExportReceiptItems(
    ...
    const materials = React.useMemo(() => exportItems || [], [exportItems]);
        ...
        () => (
            <View style={styles.tableInner}>
                ...
                {renderTableColumn(
                    ...
                    mat => (
                        <Text style={[styles.tableCell, styles.priceText]} numberOfLines={1}>
                            {formatCurrency(mat.costPrice ?? 0)}
                        </Text>
                    ),
                    'flex-end'
                )}
                {renderTableColumn(
                    ...
                    mat => (
                        <Text style={[styles.tableCell, styles.priceText]} numberOfLines={1}>
                            {formatCurrency(mat.totalAmount ?? 0)}
                        </Text>
                    ),
                    'flex-end'
                )}
            ...
            <View style={styles.container}>
                ...
                >
                    <View style={styles.infoSection}>
                        <DetailRow label="Loại phiếu:" value="Phiếu xuất kho" />
                        <DetailRow label="Người tạo:" value={item.creator?.fullname || '---'} />
                        <DetailRow
                            label="Kho xuất:"
                            value={item.warehouseName || item.pondName || '---'}
                        />
                        <DetailRow
                            label="Ngày tạo:"
