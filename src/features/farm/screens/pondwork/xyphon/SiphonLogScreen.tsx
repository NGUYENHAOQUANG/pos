...
import { SiphonLogItem } from '@/features/farm/components/pondwork/xyphon/SiphonLogItem';
...
export const SiphonLogScreen: React.FC = () => {
  ...
  const itemsByDate = useMemo(() => {
    ...
    return getPondJobItemsGroupedByDate(pond.id, 'SIPHON', startDate, endDate);
  ...
  const handleEditSiphon = (item: JobExecution) => {
    if (pond) {
    ...
    <View style={styles.container}>
      <View style={styles.headerSection}>
        <View style={[styles.header, { paddingTop: insets.top + 12 }]}>
          ...
          <Text style={styles.headerTitle}>Nhật ký xi-phông</Text>
      ...
      >
        {itemsByDate.size === 0 ? (
          <View style={styles.emptyContainer}>
            <EmptyStateCard
              message="Chưa có dữ liệu xi-phông"
              buttonTitle="Bắt đầu xi-phông"
              onPress={handleStartSiphon}
            />
          </View>
        ) : (
