...
export const convertWaterSupplyMetaToActivityData = (
  ...
  if (meta.targetLevel !== undefined) {
    data.push({ label: 'Mực nước mục tiêu', value: meta.targetLevel || '-', unit: 'cm' });
  }
  if (meta.supplyLevel !== undefined) {
    data.push({ label: 'Số cm cấp', value: meta.supplyLevel || '-', unit: 'cm' });
  }
  if (meta.drainLevel !== undefined) {
    data.push({ label: 'Mực nước xả xuống', value: meta.drainLevel || '-', unit: 'cm' });
  }
  if (meta.volumeAfterDrain !== undefined) {
    data.push({ label: 'Thể tích sau xả', value: meta.volumeAfterDrain || '-', unit: 'm³' });
  }
  if (meta.volumeSupply !== undefined) {
    data.push({ label: 'Thể tích nước cấp vào', value: meta.volumeSupply || '-', unit: 'm³' });
  }
  if (meta.volumeAfterSupply !== undefined) {
    data.push({ label: 'Thể tích nước sau cấp', value: meta.volumeAfterSupply || '-', unit: 'm³' });
  }
